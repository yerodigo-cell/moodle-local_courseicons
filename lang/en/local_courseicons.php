<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * English language strings for local_courseicons.
 *
 * @package    local_courseicons
 * @copyright  2026 Yeison Díaz
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['actions'] = 'Actions';
$string['activitytypes'] = 'Activity types';
$string['alltypes'] = 'All types';
$string['bulkdelete'] = 'Restore selected';
$string['bulkupload'] = 'Change selected';
$string['cachedef_course_css'] = 'Course CSS Cache';
$string['choosemodule'] = 'Select an activity to change its icon';
$string['courseicons:manage'] = 'Manage custom activity icons';
$string['currenticon'] = 'Icon status';
$string['customiconpreview'] = 'Custom icon preview';
$string['customized'] = 'Customized';
$string['default'] = 'Default';
$string['defaulticons'] = 'Default icons';
$string['defaulticonscoursedesc'] = 'Set a default icon for all activities of a specific type in this course. Individual activity icons will override these defaults.';
$string['deletedefault'] = 'Delete default icon';
$string['deleteicon'] = 'Delete custom icon and revert to original';
$string['deleteiconconfirm'] = 'Are you sure you want to delete this custom icon? It will revert to the default Moodle icon.';
$string['deleteselectedconfirm'] = 'Are you sure you want to delete the custom icons for the selected activities?';
$string['editicon'] = 'Change icon';
$string['globalicons'] = 'Global default icons';
$string['globaliconsdesc'] = 'Set a default icon for all activities of a specific type across the entire Moodle site. Course-specific and individual activity icons will override these defaults.';
$string['globalmanage'] = 'Global icon settings';
$string['globalmanage_help'] = 'Manage global default icons for all courses.';
$string['individualicons'] = 'Individual icons';
$string['manageicons'] = 'Customize activity icons';
$string['manageicons_help'] = 'Upload SVG, PNG, JPG or GIF files to replace the default Moodle icons in your course activities.';
$string['modulename'] = 'Activity name';
$string['nodefaults'] = 'No default icons set for this course.';
$string['noiconsfound'] = 'No icons found matching your search.';
$string['pluginname'] = 'Course Activity Icons';
$string['preview'] = 'Icon preview';
$string['privacy:metadata'] = 'The Course Activity Icons plugin does not store any personal data.';
$string['savechanges'] = 'Save changes';
$string['searchactivities'] = 'Search activities...';
$string['searchlibrary'] = 'Search library...';
$string['selectfromlibrary'] = 'Choose from library';
$string['setdefaulticon'] = 'Set default icon';
$string['successdeleted'] = 'The custom icon has been deleted.';
$string['successupdated'] = 'The activity icon has been successfully updated.';
$string['uploadcustom'] = 'Upload custom file';
$string['uploaddefaulticon'] = 'Upload default icon for: {$a}';
$string['uploadicon'] = 'SVG/Image/GIF Icon for: {$a}';
$string['uploadiconbulk'] = 'Upload SVG/Image/GIF for {$a} selected activities';
